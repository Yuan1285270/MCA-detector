...
Early social bot detection research mainly focused on account-level classification. Ferrara et al. described the rise of social bots and discussed how bots can affect political, economic, and public discussions \cite{ferrara2016rise}. Varol et al. proposed a human-bot interaction detection framework using metadata, content, sentiment, network, and temporal features \cite{varol2017online}. These methods are useful for estimating whether a single account is automated, but they are less directly suited to explaining whether multiple accounts act together.

Another line of research emphasizes group behavior and behavioral sequences. Cresci et al. proposed Social Fingerprinting, which detects groups of spambots through DNA-inspired behavioral modeling \cite{cresci2018social}. Mazza et al. introduced RTbust, which exploits temporal patterns in retweet time series to detect suspicious botnets \cite{mazza2019rtbust}. These studies show that group-level behavior and timing can reveal coordination that is not visible from single-account features alone.

\subsection{Coordination Networks and Stance Analysis}

Recent coordinated behavior detection research more directly addresses shared actions. Pacheco et al. proposed building coordination networks from shared behavioral traces and demonstrated how identity, image, hashtag sequence, retweet behavior, and temporal patterns can uncover coordinated networks \cite{pacheco2021uncovering}. Graham et al. introduced the Coordination Network Toolkit, which represents coordinated behavior using weighted directed multigraphs \cite{graham2024coordination}. In the Reddit context, Saeed et al. proposed TROLLMAGNIFIER and observed that accounts linked to troll behavior may show loose coordination, similar topics, and temporal synchronization \cite{saeed2021trollmagnifier}.

This work also builds on stance detection. The SemEval-2016 stance detection task formulated the problem of identifying whether a text is in favor of, against, or neutral toward a target \cite{mohammad2016semeval}. We extend this idea by converting comment feedback toward post authors into signed account-to-account edges and then projecting those edges into shared negative target relations.
