  ...
  en: {
    client: {
      ...
      flow: [
        ...
        ["3. Verify evidence", "Check whether accounts act together in time, writing pattern, and activation window."],
    ...
    demo: {
      ...
      guide: [
        ...
        ["2. Is the group acting together?", "Stage 2 checks timing, writing similarity, and activation-window overlap."],
      ...
      flow: [
        ...
        ["3. Multi-evidence verification", "Check temporal synchrony, text fingerprint distance, and activation window overlap."],
  ...
  zh: {
    client: {
      ...
      flow: [
        ...
        ["3. 驗證證據", "檢查帳號是否在時間、文字模式和活躍窗口上呈現共同痕跡。"],
    ...
    demo: {
      ...
      guide: [
        ...
        ["2. 這群人有沒有一起行動？", "Stage 2 檢查時間同步、文字指紋和活躍窗口重疊。"],
      ...
      flow: [
        ...
        ["3. Multi-evidence verification", "檢查 temporal synchrony、text fingerprint 和 activation window overlap。"],
...
function renderPairs() {
  ...
  const intro = currentMode === "client"
    ? currentLang === "zh"
      ? ["行為連結說明兩個帳號為什麼被放在一起看。", "時間同步越穩定，越值得優先人工審查；文字距離和活躍窗口是輔助證據。"]
      : ["Behavior links explain why two accounts should be reviewed together.", "Stable timing evidence increases review priority; text distance and activation overlap are supporting signals."]
    : currentLang === "zh"
  ...
  const headers = currentLang === "zh"
    ? ["帳號對", "時間同步", "可信度", "文字距離", "活躍重疊", "共同目標"]
    : ["Pair", "Timing", "Reliability", "Text distance", "Activation overlap", "Shared target"];
  $("#tab-pairs").innerHTML = `
    <div class="tab-intro">
      <strong>${intro[0]}</strong>
      <span>${intro[1]}</span>
