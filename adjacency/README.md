...
MCA review 不只看單一 account score。更實用的流程是：

```text
seed account -> coordination neighborhood -> reviewable group evidence
```

可以用：

```bash
.venv/bin/python adjacency/discover_coordination_groups.py \
  --seeds harvested JG87919 XBThodler
```

預設會用 `A_co_negative_target` 找小型 connected components，並把 giant components 排除在 review table 外。輸出放在：

```text
adjacency/output/group-discovery/
```
...
│   ├── group_summary.csv
│   ├── group_members.csv
│   ├── group_internal_edges.csv
│   ├── group_shared_targets.csv
│   └── skipped_large_components.csv
└── seeds/
    ├── seed_expansion_summary.csv
    └── <seed>/
        ├── coordination_neighbors.csv
        ├── trigger_neighbors.csv
        ├── internal_coordination_edges.csv
        ├── shared_negative_targets_with_seed.csv
        ├── candidate_member_features.csv
        └── summary.json
```

這些檔案是給人工 review 和 evidence tracing 用，不是最終 MCA score。

### Group Discovery Logic

`group_summary.csv` 預設從 `A_co_negative_target` 取：
...
- group members
- internal co-negative edges
- shared negative targets
- average rhetoric / oppositional / anomaly evidence
- excluded giant components

這樣可以避免單一帳號分數被大型節點或薄樣本帶偏，改成檢查：

```text
一群帳號是否共同反對同一批 target？
這群帳號內部是否彼此相連？
這群帳號是否也有 manipulative / automation evidence？
```

### Seed Expansion Logic

seed expansion 從指定帳號開始，輸出：

- top co-target neighbors
- top co-negative-target neighbors
- trigger-response in/out neighbors
- seed 與鄰居共同 negative targets
- candidate group 的 account-level evidence

這適合從 MCA ranking、EDA、或人工直覺挑一個 seed，往外看它是否真的長成一個 group。
